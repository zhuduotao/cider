...
/**
 * A copy of Alibaba chat model support from @langchain/community
 * but will support `qwen-long` model
...
type FinishReasons = "stop" | "length" | "null" | null

interface SimpleChatCompleteChunk {
  text: string;
  finish_reason: FinishReasons;
}
interface MultiChoiceChatCompleteChunk {
  choices: {
    message: {
      content: string,
      role: string
...
interface ChatCompletionResponse {
  ...
  output: SimpleChatCompleteChunk | MultiChoiceChatCompleteChunk;
}

function _isSimpleChatCompleteChunk(
  chunk: SimpleChatCompleteChunk | MultiChoiceChatCompleteChunk
): chunk is SimpleChatCompleteChunk {
  return (chunk as SimpleChatCompleteChunk).text !== undefined;
}

function _sageGetCompletion(chunk: SimpleChatCompleteChunk | MultiChoiceChatCompleteChunk): {
  text: string,
  finish_reason: FinishReasons;
} {
  if(_isSimpleChatCompleteChunk(chunk)) {
    return {
      text: chunk.text,
  ...
  implements AlibabaTongyiChatInput
{
  ...
  async _generate(
      ...
      ? await new Promise<ChatCompletionResponse>((resolve, reject) => {
        ...
        this.completionWithRetry(
          ...
          (event) => {
            ...
            const { text, finish_reason } = _sageGetCompletion(data.output);

            if (!response) {
              response = data;
            } else {
              const output = response.output as SimpleChatCompleteChunk
    ...
    const { text } = _sageGetCompletion(data.output);
  ...
  async *_streamResponseChunks(
    ...
    for await (const chunk of stream) {
      const { text, finish_reason } = _sageGetCompletion(chunk.output);
  ...
  private async *createTongyiStream(
    ...
    for await (const chunk of stream) {
      ...
      for (const line of lines) {
        ...
        try {
          const chunk = JSON.parse(line.slice("data:".length).trim());
          yield chunk
        } catch (e) {
